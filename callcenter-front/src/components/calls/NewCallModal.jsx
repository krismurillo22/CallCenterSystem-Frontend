 ...
 * - isOpen: boolean
 * - onClose: () => void
 * - onSubmit: ({ caller_name, caller_phone, rank_required }) => void
 */
export default function NewCallModal({ isOpen, onClose, onSubmit }) {
  ...
  const valid = callerName.trim().length > 0 && callerPhone.trim().length > 0 && [1,2,3].includes(rank);

  const handleSubmit = (e) => {
    e.preventDefault();
    setTouched(true);
    if (!valid) return;
    onSubmit({ caller_name: callerName.trim(), caller_phone: callerPhone.trim(), rank_required: rank });
    onClose();
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Nueva llamada entrante">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-2">Nombre del llamante</label>
          <input
            autoFocus
            value={callerName}
        ...
        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-2">Teléfono</label>
          <input
            value={callerPhone}
          ...
          {touched && callerPhone.trim().length === 0 && (
            <p className="text-xs text-red-500 mt-1">El teléfono es requerido.</p>
          )}
        </div>

        <div>
          <label className="block text-xs font-medium text-muted-foreground mb-2">Rango requerido</label>
